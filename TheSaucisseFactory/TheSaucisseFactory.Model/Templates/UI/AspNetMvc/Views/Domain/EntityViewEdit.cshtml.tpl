[%@ template enumerable='Producer.VisibleEntities' enumerableItemName="entity" enumerableTargetPathFunc='Path.Combine(Path.GetDirectoryName(TargetPath), RelativePath(entity)) + @"\Edit" + Template.FileExtension' inherits="CodeFluent.Producers.UI.BaseTemplate" %]
...
%]
@{
    ViewBag.Title = "[%=l_displayName%] - Edition";
}

<h2>[%=l_displayName%] - Edition</h2>
...
@using (Html.BeginRouteForm("[%=entity.ClrFullTypeName%]", new {controller = "[%=l_controllerName%]", action = "Edit"},
    ...
    <fieldset>
		...
		[%WriterOptions.Mode = FormMode.Edit;Write(entity.DefaultForm);%]
