...
%]		<ObjectDataProvider x:Key="[%=typedObject.ProjectEntity.Set.Name%][%=typedObject.ChoiceMethod.Name%]" MethodName="[%=typedObject.ChoiceMethod.Name%]" 
	...
	{%]
		  <ComboBox ItemsSource="{Binding  Source={StaticResource [%=typedObject.ProjectEntity.Set.Name%][%=typedObject.ChoiceMethod.Name%]}}"
			      SelectedItem="{Binding Path=[%=typedObject.Name%]}" />
	[%}%]
	</DockPanel>
    <x:Code>
        <![CDATA[
